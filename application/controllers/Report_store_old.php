...
class Report_store_old extends CI_Controller {

	public function index()
	{
		$this->load->model('report/store_report_old_model', 'store_report_old');
		$reportData = $this->store_report_old->get_store_day_total_money(1570,'2016-09-03');
		print_r($reportData);

		print_r("***********************");
		$reportData = $this->store_report_old->get_store_day_total_orderCount(1570,'2016-09-03');
		print_r($reportData);

		print_r("***********************");
		$reportData = $this->store_report_old->get_store_day_taill(1570,'2016-09-03');
		print_r($reportData);
	}
	...
	public function store_day_checkout_from($storeid)
	{
		...
		$dayData = $this->store_report_old->store_day_query($storeid);
		if (isset($dayData) && count($dayData)>0) {
			echo '<script language="javascript" type="text/javascript">alert("今日已经结过帐!");window.location.href="'.site_url("/report_store_old/store_day_checkout").'";</script>';
		}

		$dayCheck = $this->store_report_old->store_day_checkout($storeid);
		if ($dayCheck>-1) {
			echo '<script language="javascript" type="text/javascript">alert("创建结过帐成功!");window.location.href="'.site_url("/report_store_old/store_day_checkout").'";</script>';
		}
		else{
			echo '<script language="javascript" type="text/javascript">alert("创建结过帐失败!");window.location.href="'.site_url("/report_store_old/store_day_checkout").'";</script>';
			exit;
		}
	}
...
//		exit;
	...
	public function store_day_pool()
	{
		...
		$reportData = $this->store_report_old->store_day_period_query_sum($return["storeWhere"], $return["querydate"], $return["queryenddate"],$addkind);
		$return["reportData"] = $reportData;
	...
	public function store_month_pool()
	{
		...
		$reportData = $this->store_report_old->store_month_period_query($return["storeWhere"], $return["querydate"], $return["queryenddate"]);
		$return["reportData"] = $reportData;
	...
	public function get_bill_check()
	{
		...
		$reportData = $this->store_report_old->check_bill($return["storeid"],$return["querydate"]);
	...
	public function exec_close_bill()
	{
		...
		$dayData = $this->store_report_old->store_day_query($return["storeid"], $return["querydate"]);

		if (count($dayData)<1) {

			if(strtotime(date("Y-m-d",strtotime()))>strtotime(date("Y-m-d",strtotime($return["querydate"])))){
				echo '<script language="javascript" type="text/javascript">alert("操作时间不能大于当前时间!");window.location.href="'.site_url("/report_store_old/close_bill_log").'";</script>';
				return;
			}

			$reportData = $this->store_report_old->bill_date_store($return["storeid"],$return["querydate"],$return["userid"]);
			$return["reportData"] = $reportData;

			if (count($reportData) >0) {
				echo '<script language="javascript" type="text/javascript">alert("'.$return["querydate"].', 封帐成功! 操作人:'.$return["username"].'");window.location.href="'.site_url("/report_store_old/close_bill_log").'";</script>';
				return;
			}
		}
		else{
			echo '<script language="javascript" type="text/javascript">alert("'.$return["querydate"].'已经封过帐! 不可以重复封帐!");window.location.href="'.site_url("/report_store_old/close_bill").'";</script>';
			return;
		}

		echo '<script language="javascript" type="text/javascript">alert("封帐失败! 请联系系统管理员!");window.location.href="'.site_url("/report_store_old/close_bill").'";</script>';
	}
	...
	public function close_bill_log()
	{
		...
		$reportData = $this->store_report_old->query_date_bill_store_log($return["storeid"],$return["querydate"]);
		$return["reportData"] = $reportData;
	...
	public function classify_performance()
	{
		...
		$reportData = $this->store_report_old->query_classify_performance($return["storeid"],$return["querydate"], $return["queryenddate"]);
		$return["reportData"] = $reportData;
	...
	public function  update_order_payte($storeid)
	{
		...
		$this->store_distribution->redistribution_order_account($oid);
		$jsonReturn = array();
		$jsonReturn["code"] = "000";
	...
	public function classify_pool()
	{
		...
		$reportData = $this->store_report_old->store_day_period_query($return["storeid"], $return["querydate"], $return["queryenddate"]);
		$return["reportData"] = $reportData;
