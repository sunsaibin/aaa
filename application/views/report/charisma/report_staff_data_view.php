...
<div > 按员工编号进行搜索(双击清空)：<input name="seachkey" type="text" id="seachkey" onkeydown="onSearch(this)" value="" style="margin-bottom:10px;"/></div>
	<table  id="storePermance" style="max-width:1850px;">
		<tbody>
			<tr>
				<th>门店名称</th>
				<td>时间</td>
			...
			if(!empty($reportData))
			{
				 foreach ($reportData as $key => $v) {
					...
					echo '<td>'.$v->year.'/'.$v->month.'/'.$v->day.'<br/>'.$v->enddata.'</td>';
					echo '<td>'.$v->staff_number.'/'.$v->staffid.'</td>';
					echo '<td><a href="'.site_url("/Report_staff_old/salary_detail/".$v->staffid)."?storeid=".$v->staffid."&queryend=".str_replace("/","-",$v->enddata)."&querydate=".$querydate.'">'.$v->staff_name.'</a></td>';
...
function onSearch(obj){//js函数开始
	setTimeout(function(){//因为是即时查询，需要用setTimeout进行延迟，让值写入到input内，再读取
		var storeId = document.getElementById('storePermance');//获取table的id标识
