                ...
                <tbody>
                    ...
                    if(!empty($reportData))
                    {
                        foreach ($reportData as $key => $v) {
                            ...
                            //绘制表格
                            echo '<td>'.$v->STAFFNUMBER.' / '.$v->STAFFID.'</td>';
                            echo '<td style="font-size:10px;color:black;">'.$v->STAFFNAME.'</td>';
                            if ($v->deduct_type == 0) {
                                echo '<td style="font-size:10px;color:black;">'.$v->product_name.'</td>';
                            }
							else{
                                echo '<td style="font-size:10px;color:black;">'.$v->card_id.'</td>';
                            }
                            echo '<td>'.$v->bonus_time.'</td>';
                            echo '<td>'.$v->orderproduct_id.'/'.$v->order_id.'</td>';
                            echo '<td style="font-size:10px;color:black;">'.sprintf("%.2f",$v->total_performance).'</td>';
                            echo '<td style="font-size:10px;color:black;">'.sprintf("%.2f",$v->performance_amount).'</td>';
							echo '<td style="font-size:10px;color:black;">'.sprintf("%.3f",$v->performance_ratio).'</td>';
							echo '<td style="font-size:10px;color:black;">'.sprintf("%.2f",$v->staff_performance).'</td>';
                            echo '<td style="font-size:10px;color:black;">'.sprintf("%.3f",$v->staff_performance_ratio).'</td>';
                            echo '<td style="font-size:10px;color:black;">'.sprintf("%.2f",$v->commission_amount).'</td>';
                            echo '<td style="font-size:10px;color:black;">'.sprintf("%.3f",$v->commission_ratio).'</td>';
                            echo '</tr>';
                        }
                    }
                    ?>
