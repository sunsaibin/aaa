...
<script type="text/javascript">
    ...
    function request_data(){
        ...
        $.post("<?php echo site_url(); ?>/store_report/store_period_daily_report_data",
    ...
    {
        //jQuery("#table_list_1").clearGridData();   //清空原grid数据
        //jQuery("#table_list_1").addJSONData(items_data);
        $("#table_list_1").jqGrid("clearGridData");
        $("#table_list_1").jqGrid("setGridParam",{datatype:"local",data:items_data}).trigger("reloadGrid");
//        for ( var i = 0; i <= items_data.length; i++){
//            jQuery("#table_list_1").jqGrid('addRowData', i + 1, items_data[i]);
//        }
    }
</script>

<script>
    var dm_table_header = ['日期/门店', '总收入', '实业绩', '总订单数', '服务业绩', '产品业绩', '卡异动业绩','服务','产品','售卡','退卡','合计','服务 ','产品','售卡','退卡','合计','服务 ','产品','售卡','退卡','合计','服务 ','产品','售卡','退卡','合计','大众点评 ','百度糯米','合计','服务','产品','合计','服务','产品','合计','服务','产品','合计','售卡','退卡','合计','经理签单','支出登记','疗程兑换','储值兑换'];
    $(document).ready(function () {
        ...
        $("#table_list_1").jqGrid({
            ...
            {
//                var total_col2=$("#table_list_1").getCol('col2',false,'sum');
//                var total_col3=$("#table_list_1").getCol('col3',false,'sum');
//                var total_col4=$("#table_list_1").getCol('col4',false,'sum');
                var exclude_array = ["1"];
               ...
               // $("#table_list_1").footerData('set', { "col1": '合计:', col2: total_col2, col3: total_col3,col4:total_col4 });
        ...
        $("#table_list_1").jqGrid('setGroupHeaders', {
            ...
            groupHeaders:[
                {startColumnName: 'col8', numberOfColumns:5, titleText: '现金支付',align:"center"},
                {startColumnName: 'col13', numberOfColumns: 5, titleText: '银行卡支付',align:"center"},
...
<script>
    function  exportDmExcel(tableId){
        var excel_table = $("#"+tableId);
...
<script>
    function export_dmExcel(tableId)
    {
        ...
        var tableHeader = ['日期/门店', '总收入', '实业绩', '总订单数', '服务业绩', '产品业绩', '卡异动业绩','现金支付服务','现金支付产品','现金支付售卡','现金支付退卡','现金支付合计','银行卡支付服务 ','银行卡支付产品','银行卡支付售卡','银行卡支付退卡','银行卡支付合计','支付宝支付服务 ','支付宝支付产品','支付宝支付售卡','支付宝支付退卡','支付宝支付合计','微信支付服务 ','微信支付产品','微信支付售卡','微信支付退卡','微信支付合计','第三方支付大众点评 ','第三方支付百度糯米','第三方支付合计','销卡支付服务','销卡支付产品','销卡支付合计','优惠券服务','优惠券产品','优惠券合计','转账服务','转账产品','转账合计','卡异动售卡','卡异动退卡','卡异动合计','经理签单','支出登记','疗程兑换','储值兑换'];
